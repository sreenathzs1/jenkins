def nexus(COMPONENT) {
    ...
    command = "curl -f -v -u admin:admin123 --upload-file /home/ubuntu/workspace/CI-Pipelines/${COMPONENT}-ci/${FILENAME} http://172.31.11.166:8081/repository/${COMPONENT}/${FILENAME}"
...
def make_artifacts(APP_TYPE, COMPONENT) {
    ...
    if(APP_TYPE == "NGINX") {
        ...
        command = "cp target/*.jar ${COMPONENT}.jar && zip -r ${FILENAME}.zip ${COMPONENT}.jar"
        def execute_com=sh(returnStdout:true, script: command)
        print execute_com
        ...
        command = "zip -r ${FILENAME}.zip node_modules server.js"
        def execute_com=sh(returnStdout:true, script: command)
        print execute_com
    }
}
